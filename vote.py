...
def login():
    ...
    aadhar_input   = (data.get('aadhar')   or '').strip()
    voter_id_input = (data.get('voter_id') or '').strip().upper()
    name_input     = (data.get('name')     or '').strip().lower()

    # Step 1: look up by Aadhaar only
    voter = Voter.query.filter_by(aadhar_number=aadhar_input).first()

    if not voter:
        return jsonify({"status": "error", "message": "No voter record found for this Aadhaar number."}), 404

    # Step 2: cross-check Voter ID
    if voter.voter_id_card.upper() != voter_id_input:
        return jsonify({"status": "error", "message": "Voter ID does not match our records."}), 401

    # Step 3: cross-check Name (case-insensitive)
    if voter.name.strip().lower() != name_input:
        return jsonify({"status": "error", "message": "Name does not match our records."}), 401

    if voter.has_voted:
        return jsonify({"status": "error", "message": "This voter has already cast their vote."}), 403

    # All three match — issue token
